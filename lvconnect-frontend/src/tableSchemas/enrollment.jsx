...
export const registrarSchema = {
  ...
  stat: {
    ...
    customCell: (value, original) => {
      ...
      const map = {
        gnot_enrolled: <span style={{ color: "gray" }}>Not Enrolled</span>,
...
export const actions = (openModal, openAcceptModal, openRejectModal, tab) => ({
  ...
  accept: {
    icon: (item) => {
      if (item.status === "pending" && tab === "pending") {
        return (
  ...
  reject: {
    ...
    variant: (item) => "default",
    className: "hover:bg-red-200 flex px-2 py-1 text-xs sm:text-sm max-w-xs"
  },
...
export const actionConditions = {
  ...
  accept: (item, context, userRole) => item.status === 'pending',
  reject: (item, context, userRole) => item.status === 'pending',
};
