  ...
  return (
    <div className="box-border flex flex-col items-start p-2.5 gap-5 w-full max-w-full border border-solid border-[#CED4DA] rounded-lg mx-auto">
      <div className="flex flex-col items-start w-full">
        <h5 className="font-lato font-semibold text-lg text-[#1F3463] mb-1">
      ...
      {showHandbookModal && (
        <StudentHandbook isOpen={() => setShowHandbookModal(true)} onClose={() => setShowHandbookModal(false)} />
      )}
       {showPrivacyModal && (
        <PrivacyPolicy isOpen={() => setShowPrivacyModal(true)} onClose={() => setShowPrivacyModal(false)} />
      )}
    </div>
  );
}
